...
const COLORS = ["#3b82f6", "#f59e0b", "#10b981"] // أزرق - أصفر - أخضر

export default function DashboardPage() {
  const { tasks, addTask, updateTask, deleteTask, getTasksByStatus } = useTaskStore()
    ...
    <div className="container mx-auto py-10 px-4 max-w-7xl" dir="rtl">
      <div className="flex items-center justify-between mb-12">
        ...
        <div className="text-sm text-muted-foreground">
          إجمالي المهام: <span className="font-medium">{total}</span>
        </div>
      </div>
      ...
      <Card className="mb-16 border-none shadow-lg">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* باقي النموذج زي ما هو في النسخة السابقة */}
            <Card className="mb-16 border-none shadow-lg">
              <CardHeader>
                <CardTitle className="flex items-center gap-3">
                  {editingTask ? <Edit2 className="h-6 w-6" /> : <Plus className="h-6 w-6" />}
      ...
      {/* باقي الأعمدة الثلاثة نفسها مع التحسينات السابقة */}
          ...
          <Card key={key} className="border-none shadow-lg">
            <CardHeader className="flex flex-row items-center gap-3 pb-2">
              ...
              <Badge variant="secondary" className="ml-auto">
                {getTasksByStatus(key as any).length}
              </Badge>
            </CardHeader>
            <CardContent className="space-y-4 min-h-[400px]">
              {getTasksByStatus(key as any).length === 0 ? (
                <div className="h-full flex items-center justify-center text-muted-foreground">
                  لا توجد مهام هنا حاليًا
                </div>
              ) : (
                getTasksByStatus(key as any).map(task => (
